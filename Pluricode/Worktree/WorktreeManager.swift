...
final class WorktreeManager {
    ...
    func listWorktrees() async throws -> [WorktreeInfo] {
        let result = try await run("git", args: ["-C", repoRoot.path, "worktree", "list", "--porcelain"])
    ...
    func listManagedWorktrees() async -> [Worktree] {
        ...
        if let primary = all.first(where: {
            ...
            result.append(Worktree(
                branch: primary.branch.isEmpty ? await defaultBranch() : primary.branch,
                path: primary.path,
                head: primary.head,
                isPrimary: true
            ))
    ...
    func renameWorktree(oldBranch: String, newName: String) async throws -> Worktree {
        guard newName != oldBranch else {
            let info = try await listWorktrees().first { $0.branch == oldBranch }
        ...
        guard let info = try await listWorktrees().first(where: { $0.branch == oldBranch }) else {
        ...
        if oldPath.standardizedFileURL != newPath.standardizedFileURL {
            let move = try await run("git", args: ["-C", repoRoot.path, "worktree", "move", oldPath.path, newPath.path])
            if move.status != 0 {
                throw WorktreeError.createFailed(move.stderr)
            }
        }

        let rename = try await run("git", args: ["-C", repoRoot.path, "branch", "-m", oldBranch, newName])
        if rename.status != 0 {
            throw WorktreeError.createFailed(rename.stderr)
        }

        let updated = try await listWorktrees().first { $0.branch == newName }
    ...
    static func isMerged(at path: URL) async -> Bool {
        guard let branch = try? await run("git", args: [
        ...
        guard let result = try? await ProcessRunner.run("gh", args: [
            "pr", "list", "--state", "merged", "--head", name, "--limit", "1", "--json", "number"
        ], cwd: path, timeout: 30), result.status == 0 else { return false }

        let out = result.stdout.trimmingCharacters(in: .whitespacesAndNewlines)
        return !out.isEmpty && out != "[]"
    }

    static func diffStats(at path: URL) async -> DiffStats {
        guard let base = await baseCommit(at: path) else { return .zero }
        var adds = 0
        var dels = 0
        if let result = try? await run("git", args: [
    ...
    private static func baseCommit(at path: URL) async -> String? {
        var base = "main"
        if let result = try? await run("git", args: [
        ...
        guard let result = try? await run("git", args: [
    ...
    func currentBranch(at path: URL) async -> String? {
        guard let result = try? await run("git", args: ["-C", path.path, "rev-parse", "--abbrev-ref", "HEAD"]),
              result.status == 0 else { return nil }
    ...
    func defaultBaseRef() async -> String {
        if let result = try? await run("git", args: [
            "-C", repoRoot.path,
            "symbolic-ref", "refs/remotes/origin/HEAD", "--short"
        ]), result.status == 0 {
        ...
        return await defaultBranch()
    }

    static func findRepoRoot(from path: URL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)) async -> URL? {
        guard let result = try? await run("git", args: ["-C", path.path, "rev-parse", "--show-toplevel"]),
              result.status == 0 else { return nil }
        let root = result.stdout.trimmingCharacters(in: .whitespacesAndNewlines)
        return root.isEmpty ? nil : URL(fileURLWithPath: root)
    }

    private static func run(_ executable: String, args: [String], cwd: URL? = nil) async throws -> ProcessResult {
        try await ProcessRunner.run(executable, args: args, cwd: cwd)
    }

    private func run(_ executable: String, args: [String], cwd: URL? = nil) async throws -> ProcessResult {
        try await ProcessRunner.run(executable, args: args, cwd: cwd)
    }
}
